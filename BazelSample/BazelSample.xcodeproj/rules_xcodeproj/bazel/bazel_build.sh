...
readonly bazel_cmd=(
  ...
  "/Users/andre.alves/Library/Caches/bazelisk/downloads/bazelbuild/bazel-6.2.1-darwin-arm64/bin/bazel"
...
"$BAZEL_INTEGRATION_DIR/process_bazel_build_log.py" \
  ...
  "$output_groups_flag" \
  "@rules_xcodeproj_generated//generator/BazelSample/BazelSampleXcodeProj:BazelSampleXcodeProj" \
...
if [[ -n "${target_ids:-}" ]]; then
  diff_output=$(comm -23 <(printf '%s\n' "${target_ids[@]}") "$PROJECT_DIR/bazel-out/darwin_arm64-dbg/bin/external/rules_xcodeproj_generated/generator/BazelSample/BazelSampleXcodeProj/BazelSampleXcodeProj_target_ids")
